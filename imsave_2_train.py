...
mhaus_writer = tf.summary.FileWriter('./' + input_parameters['test_name'] + '_checkpoints/logs/haus/meanval', sess.graph)
mjacc_writer = tf.summary.FileWriter('./' + input_parameters['test_name'] + '_checkpoints/logs/iou/meanval', sess.graph)

train_image_writer = tf.summary.FileWriter('./' + input_parameters['test_name'] + '_checkpoints/logs/images/train', sess.graph)
val_image_writer = tf.summary.FileWriter('./' + input_parameters['test_name'] + '_checkpoints/logs/images/val', sess.graph)
    ...
    for i in range(num_iters):
        ...
        for trainimage in range(train_im.shape[0]):
            ...
            if train_save_ON == True:
                ...
                train_image_saver = tf.summary.image('Training Images Epoch %i' % epoch, imgset_tosave)
                train_image_writer.add_summary(train_image_saver.eval(session=sess), traincount)
    ...
    if epoch % input_parameters['validation_step'] == 0:
        ...
        for ind in val_indices:
            ...
            if (save_ON == 1):
                ...
                print(val_im_tosave.shape)

                imgset_tosave = np.expand_dims(val_im_tosave, axis=3)

                val_image_saver = tf.summary.image('Validation Images Epoch %i' % epoch, imgset_tosave)
